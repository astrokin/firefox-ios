...
//        waitForExistence(app.cells["action_pin"])
    ...
    func testSendToDeviceFromPageOptionsMenu() {
        ...
        XCTAssertTrue(app.staticTexts["You are not signed in to your Decentr Account."].exists)
    }
    ...
    func testSharePageWithShareSheetOptions() {
        openNewShareSheet()
        waitForExistence(app.staticTexts["Open in Decentr"], timeout: 10)
        XCTAssertTrue(app.staticTexts["Open in Decentr"].exists)
