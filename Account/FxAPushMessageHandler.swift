        ...
        RustFirefoxAccounts.reconfig(prefs: profile.prefs).uponQueue(.main) { accountManager in
            accountManager.deviceConstellation()?.processRawIncomingAccountEvent(pushPayload: string) {
                ...
                if events.count > 1 {
                    ...
                    os_log("%{public}@", log: OSLog(subsystem: "org.mozilla.decentr", category: "firefoxnotificationservice"), type: OSLogType.debug, "Multiple events arrived, only handling the first event.")
                }
                switch firstEvent {
