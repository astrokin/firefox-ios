...
public extension UIDevice {
    private static var systemId: String {
        var systemInfo = utsname()
        uname(&systemInfo)
        let machineMirror = Mirror(reflecting: systemInfo.machine)
        let children = machineMirror.children
        let identifier: String = children.reduce("") { identifier, element -> String in
            guard let value = element.value as? Int8, value != 0 else { return identifier }
            let uint = UInt8(value)
            let scalar = UnicodeScalar(uint)
    ...
    static let modelName: String = {
        mapToDevice(identifier: systemId)
    }()
}
